...
const CONFIDENCE_RUBRIC = `   a. 0: Not confident at all. This is a false positive that doesn't stand up to light scrutiny, or is a pre-existing issue.
   b. 25: Somewhat confident. This might be a real issue, but may also be a false positive. The agent wasn't able to verify that it's a real issue. If the issue is stylistic, it is one that was not explicitly called out in the relevant AGENTS.md/CLAUDE.md.
   ...
   d. 75: Highly confident. The agent double checked the issue, and verified that it is very likely it is a real issue that will be hit in practice. The existing approach is insufficient. The issue is very important and will directly impact the code's functionality, or it is an issue that is directly mentioned in the relevant AGENTS.md/CLAUDE.md.
...
- General code quality issues (eg. lack of test coverage, poor documentation), unless explicitly required in AGENTS.md/CLAUDE.md
...
2. Use another lightweight subagent to give you a list of file paths to (but not the contents of) any relevant agent guidance files from the codebase: AGENTS.md (preferred) and CLAUDE.md (fallback). Look for the root AGENTS.md or CLAUDE.md (if one exists), as well as any in the directories whose files the pull request modified. Prefer AGENTS.md over CLAUDE.md when both exist in the same location.
...
- You must cite and link each bug (eg. if referring to an AGENTS.md/CLAUDE.md, you must link it).
...
1. <brief description of bug> (severity: blocker) (AGENTS.md/CLAUDE.md says "<...>")
...
2. <brief description of bug> (severity: major) (some/other/AGENTS.md/CLAUDE.md says "<...>")
...
2. Use a lightweight subagent to give you a list of file paths to (but not the contents of) any relevant agent guidance files from the codebase: AGENTS.md (preferred) and CLAUDE.md (fallback). Look for the root AGENTS.md or CLAUDE.md (if one exists), as well as any in the directories whose files are under review. Prefer AGENTS.md over CLAUDE.md when both exist in the same location.
...
1. <brief description> (severity: blocker) (AGENTS.md/CLAUDE.md says "<...>")
   - <file>:<line> — <impact and suggested fix>
...
11. Write the review report to a markdown document in English (regardless of any language guidance in AGENTS.md/CLAUDE.md), following the REPORT_TEMPLATE below, and also write a machine-readable JSON file:
