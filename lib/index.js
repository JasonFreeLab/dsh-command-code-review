 ...
 * - /code-review [request] (or empty) — local review of the requested scope or the current uncommitted changes, with an optional report document (--out <dir>, default "doc").
 *
...
<list each confirmed issue; for each, state the file and line, the defect, the impact, and a suggested fix. Example:>

1. <brief description> (<reason: dsh.md / bug / git history / prior review comment / code comment>)
   - <file>:<line> — <impact and suggested fix>

<or, if no issues passed the threshold:>

No issues found. Checked for bugs and dsh.md compliance.`;

const WORKFLOW_PR = `Provide a code review for the given pull request. To do this, follow these steps precisely:
...
4. Then, launch 5 parallel thorough subagents in the background (set run_in_background: true so they run concurrently) to independently code review the change. The agents should do the following, then return a list of issues and the reason each issue was flagged (eg. dsh.md adherence, bug, historical git context, etc.):
   a. Agent #1: Audit the changes to make sure they comply with the dsh.md. Note that dsh.md is guidance for the agent as it writes code, so not all instructions will be applicable during code review.
...
Launch all 5 subagents in one batch, then wait for the runtime to notify you when each one finishes — you are notified automatically when a background subagent settles, so do NOT poll their status with list_agents or repeated checks. This also applies to every parallel-subagent step below (including step 5). Proceed to step 5 only after all 5 have reported.

5. For each issue found in #4, launch a parallel lightweight subagent that takes the PR, issue description, and list of dsh.md files (from step 2), and returns a score to indicate the agent's level of confidence for whether the issue is real or false positive. To do that, the agent should score each issue on a scale from 0-100, indicating its level of confidence. For issues that were flagged due to dsh.md instructions, the agent should double check that the dsh.md actually calls out that issue specifically. The scale is (give this rubric to the agent verbatim):
   ...
   d. 75: Highly confident. The agent double checked the issue, and verified that it is very likely it is a real issue that will be hit in practice. The existing approach in the PR is insufficient. The issue is very important and will directly impact the code's functionality, or it is an issue that is directly mentioned in the relevant dsh.md.
   e. 100: Absolutely certain. The agent double checked the issue, and confirmed that it is definitely a real issue, that will happen frequently in practice. The evidence directly confirms this.
6. Filter out any issues with a score less than CONFIDENCE_THRESHOLD. If there are no issues that meet this criteria, do not proceed.
7. Use a lightweight subagent to repeat the eligibility check from #1, to make sure that the pull request is still eligible for code review.
8. Finally, use the gh command (through the bash tool) to comment back on the pull request with the result. When writing your comment, keep in mind to:
...
- General code quality issues (eg. lack of test coverage, general security issues, poor documentation), unless explicitly required in dsh.md
...
1. <brief description of bug> (dsh.md says "<...>")
...
2. <brief description of bug> (some/other/dsh.md says "<...>")
...
const WORKFLOW_LOCAL = `Review the code for the request below. To do this, follow these steps precisely:
...
4. Then, launch 5 parallel thorough subagents in the background (set run_in_background: true so they run concurrently) to independently review the scope. The agents should do the following, then return a list of issues and the reason each issue was flagged (eg. dsh.md adherence, bug, historical git context, etc.):
   a. Agent #1: Audit the scope for dsh.md compliance. Note that dsh.md is guidance for the agent as it writes code, so not all instructions will be applicable during code review.
   b. Agent #2: Do a shallow scan of the scope for obvious bugs. Avoid reading extra context beyond the scope, focusing just on the code under review. Focus on large bugs, and avoid small issues and nitpicks. Ignore likely false positives.
   c. Agent #3: Read the git blame and history of the code under review, to identify any bugs in light of that historical context.
   d. Agent #4: Read previous changes (commits or pull requests) that touched these files, and check for any comments on those changes that may still apply.
   e. Agent #5: Read code comments in the files under review, and make sure the code complies with any guidance in the comments.

Launch all 5 subagents in one batch, then wait for the runtime to notify you when each one finishes — you are notified automatically when a background subagent settles, so do NOT poll their status with list_agents or repeated checks. This also applies to every parallel-subagent step below (including step 5). Proceed to step 5 only after all 5 have reported.

5. For each issue found in #4, launch a parallel lightweight subagent that takes the request, the issue description, and the list of dsh.md files (from step 2), and returns a score to indicate the agent's level of confidence for whether the issue is real or false positive. To do that, the agent should score each issue on a scale from 0-100, indicating its level of confidence. For issues that were flagged due to dsh.md instructions, the agent should double check that the dsh.md actually calls out that issue specifically. The scale is (give this rubric to the agent verbatim):
   a. 0: Not confident at all. This is a false positive that doesn't stand up to light scrutiny, or is a pre-existing issue.
   b. 25: Somewhat confident. This might be a real issue, but may also be a false positive. The agent wasn't able to verify that it's a real issue. If the issue is stylistic, it is one that was not explicitly called out in the relevant dsh.md.
   ...
   e. 100: Absolutely certain. The agent double checked the issue, and confirmed that it is definitely a real issue, that will happen frequently in practice. The evidence directly confirms this.
6. Filter out any issues with a score less than CONFIDENCE_THRESHOLD. If there are no issues that meet this criteria, do not proceed.
7. Report the confirmed issues directly in your reply (no gh CLI and no pull-request comment, since this is not a PR). For each issue, state the file and line, the defect, the impact, and a suggested fix, with a link to the code where possible.
...
1. <brief description of the issue> (dsh.md says "<...>")
   - <file>:<line> — <impact and suggested fix>

2. <brief description of the issue> (bug due to <file and code snippet>)
   - <file>:<line> — <impact and suggested fix>

3. <brief description of the issue> (<reason>)
   - <file>:<line> — <impact and suggested fix>
...
8. Write the review report to a markdown document in English (regardless of any language guidance in dsh.md), following the REPORT_TEMPLATE below:
   ...
   d. Filename: code-review-<sha7>-<slug>.md when a sha was obtained, otherwise code-review-<slug>.md.
   e. Write the report with the write tool to <output directory>/<filename>, and mention the saved path in your final reply.
...
const EMPTY_INPUT_PROBE = `First check whether the current branch has an open pull request (run "gh pr view" through the bash tool). If it does, review that pull request instead — switch to the pull-request workflow (eligibility check → 5 parallel review lenses → confidence scoring → post the result back to the PR with gh). If there is no open PR, review the current uncommitted changes locally.
...
export function apply(ctx, config = {}) {
  ...
  const workflowPr = renderWorkflow(WORKFLOW_PR, threshold);
  const workflowLocal = renderWorkflow(WORKFLOW_LOCAL, threshold);
